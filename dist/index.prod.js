...
async function parseDKBPDF(pdfBuffer) {
  let text2;
  try {
    text2 = await extractTextFromPDF(pdfBuffer);
  } catch (error) {
    throw new Error("PDF konnte nicht analysiert werden. Bitte stellen Sie sicher, dass es sich um eine g\xFCltige DKB-PDF handelt.");
  }
  try {
    ...
    throw new Error(`PDF konnte nicht analysiert werden: ${errorMessage}. Bitte stellen Sie sicher, dass es sich um eine g\xFCltige DKB-Wertpapierabrechnung handelt.`);
  }
}
...
async function getEurUsdRate(apiKey2) {
  const now = Date.now();
  ...
  try {
    const response = await fetch(
      `https://api.twelvedata.com/exchange_rate?symbol=EUR/USD&apikey=${apiKey2}`
    );
...
async function fetchLivePricesTwelveData(tickers, apiKey2) {
  const results = [];
  if (!apiKey2) {
    console.error("Twelve Data API key not provided");
    return results;
  }
  const eurUsdRate2 = await getEurUsdRate(apiKey2);
  const batchSize = 7;
  ...
  for (let i = 0; i < tickers.length; i += batchSize) {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.twelvedata.com/quote?symbol=${symbolsForApi.join(",")}&apikey=${apiKey2}`
      );
      if (!response.ok) {
...
var appRouter = router({
  ...
  prices: router({
    ...
    fetchTwelveData: protectedProcedure.input(z2.object({ tickers: z2.array(z2.string()) })).mutation(async ({ ctx, input }) => {
      const apiKey2 = process.env.TWELVE_DATA_API_KEY;
      if (!apiKey2) {
        throw new Error("Twelve Data API Key nicht konfiguriert. Bitte in den Einstellungen hinterlegen.");
      }
      const prices = await fetchLivePricesTwelveData(input.tickers, apiKey2);
      const positions = await getPortfolioPositions(ctx.user.id);
      let updatedCount = 0;
      let skippedCount = 0;
      for (const priceData of prices) {
...
4. Erstelle dann eine NEUE Sparplan-Verteilung f\xFCr alle empfohlenen ETFs.` : "";
    ...
    resetTemplates: protectedProcedure.mutation(async () => {
      ...
      await db.delete(aiQuestionTemplates2);
      for (const template of defaultTemplates) {
        await db.insert(aiQuestionTemplates2).values(template);
      }
...
Es wurden keine neuen Transaktionen hinzugef\xFCgt.`
        ...
        const errorMessage = error instanceof Error ? error.message : "Unerwarteter Fehler beim Importieren der PDF.";
        return {
          success: false,
          duplicate: false,
          message: errorMessage
