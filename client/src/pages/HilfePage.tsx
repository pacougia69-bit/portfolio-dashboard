...
const faqData: FAQItem[] = [
...

  {
    ...
    answer: "Der Wächter ist ein Knopf auf der Portfolio-Seite (Karte über der Aktien-Ampel). Er prüft auf Knopfdruck den Trend aller deiner Positionen (Kurs gegen SMA 50 und SMA 200, das sind gleitende Durchschnitte) und zeigt, was sich seit dem letzten Lauf geändert hat. Er bewegt kein Geld und gibt keine Kauf- oder Verkaufsanweisung – nur Handlungsstufen wie 'Beobachten' oder 'Kein neues Geld – selbst prüfen'. Ein Volllauf dauert wegen des Kursabruf-Limits ca. 5 Minuten, das Fenster bitte offen lassen. Unter dem Signal steht 'Kurs vom TT.MM.JJJJ' – ist der Kurs älter als vier Tage, wird das Datum orange ('veraltet'). Das gleiche Datum steht auch im KI-Text. Positionen, die wegen des Limits (8 Abrufe pro Minute) fehlschlagen, holt der Wächter nach einer Pause automatisch nach (bis zu zweimal). Positionen mit demselben Ticker kosten nur einen Abruf. ETFs holt der Wächter zuerst bei Yahoo Finance mit dem echten Kurs des ETFs (im Ergebnis steht dann 'Quelle: Yahoo Finance', bei Invesco All-World mit dem Zusatz 'Notierung FWRA.MI' – das ist derselbe ETF an der Mailänder Börse in Euro); nur wenn Yahoo den ETF nicht kennt, nimmt er einen ähnlichen US-Wert als Näherung (dann steht 'Näherung' mit dem US-Ticker dabei). Deutsche Xetra-Werte (z. B. Rheinmetall) kann der Kursanbieter Twelve Data im Gratis-Plan nicht liefern – auch dort hilft Yahoo Finance als Ausweichquelle. Findet auch Yahoo nichts, steht die Position unter 'Ohne Daten'. Auf der Startseite steht 'Wächter zuletzt geprüft vor N Tagen' – nach 7 Tagen wird die Zeile orange als stille Erinnerung."
  },
  {
    category: "ampel",
  ...
  },
  {
    ...
    answer: "Bei Positionen mit Gelb oder Rot erzeugt der Knopf einen fertigen Text mit Kennzahlen (mit Währung und dem Datum des Laufs), deiner Strategie-Rolle, bei Einzelaktien deiner gespeicherten Einstiegs-These und Exit-These aus der Einstiegsanalyse und – bei den KI-ETFs – deinen Kill-Kriterien (Ausstiegs-Kriterien). Die Fragen passen sich an: ETFs werden nach Kosten, Zusammensetzung und Index gefragt, Einzelaktien nach Geschäftsmodell, Quartalszahlen und Bewertung, Kryptowerte nach Produkt und Regulierung. Im Fenster gibt es einen Schalter 'Meine übrigen Positionen mitgeben' (nur Namen, keine Beträge, damit die KI Überschneidungen prüfen kann) – er steht immer auf 'aus', du entscheidest bei jedem Text neu. Wenn du einen Text kopiert hast, merkt sich das Dashboard das: In der Liste steht dann 'Text kopiert' mit Datum und Uhrzeit, der Knopf heißt 'Nochmal ansehen', und im Fenster steht ein Hinweis – so weißt du, welche Texte du schon in eine KI gegeben hast. Bei einem neuen Lauf beginnt die Markierung von vorn. Du kopierst ihn und fügst ihn in eine KI deiner Wahl ein (Gemini, ChatGPT, Claude). Die App ruft selbst keine KI auf. Die KI soll Kosten, Zusammensetzung und Wirtschaftsumfeld prüfen, aber keine Kauf- oder Verkaufsanweisung geben. Prüfe ihre Angaben immer selbst, KI erfindet manchmal Zahlen."
  },

  {
    category: "ampel",
    question: "Was ist die Auswertung beim Wächter?",
    answer: "Die Auswertung steht oben in der Wächter-Karte und fasst zusammen, was du mit den KI-Antworten herausgefunden hast – auf einer Seite statt in vielen langen Texten. Ablauf: Bei einer Position auf 'Genauer ansehen' klicken, den Text in eine KI kopieren (er endet mit einem festen Beurteilungsblock), die Antwort der KI unten im Fenster einfügen und speichern. Das Dashboard erkennt Grundlage (intakt, verschlechtert, unklar) und Datenlage (gut, widersprüchlich, dünn); was es nicht erkennt, wählst du selbst. Daraus wird eine Einordnung nach festen Regeln: 'Selbst prüfen – dringend' (Grundlage verschlechtert), 'Selbst prüfen' (Grundlage unklar oder Daten widersprüchlich/dünn oder Trend fehlt; bei den drei Themen-ETFs mit Rot auch deine Regel 'ROT → halbieren'), 'Kein neues Geld, beobachten' (Grundlage intakt, Trend Rot), 'Beobachten' (intakt, Gelb) und 'Ruhig' (intakt, Grün). Das sind Handlungsstufen, keine Kauf- oder Verkaufsempfehlungen und keine Anlageberatung. Die Einordnung nutzt immer den aktuellen Trend. Eine Analyse wird orange 'veraltet', wenn sie älter als 30 Tage ist oder sich die Trendfarbe seitdem geändert hat. Zeile anklicken zeigt Begründung und den eingefügten KI-Text (Rohtext); du kannst eine Analyse dort löschen oder eine neue speichern. Positionen mit Gelb oder Rot ohne Analyse stehen unter 'Noch nicht ausgewertet'."
  },
